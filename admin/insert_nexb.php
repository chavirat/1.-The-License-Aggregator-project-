...
echo "<strong>owner notes = </strong>".$owner_notes."<hr>";
// insert nexb_license query
$insert_nexb = "INSERT INTO nexb_license(nexb_key,nexb_name,	short_name,
  guidance,	category,	license_type,	license_profile,	license_style	,owner,
  SPDX_key,	publication_year,	keywords,	special_obligations	,homepage_url,
  text_urls,	osi_url,	faq_url	,guidance_url,	other_urls,	owner_name,
  owner_homepage_url,	owner_type,	contact_information,alias,owner_notes)
VALUES('$nexb_key','$nexb_name','$short_name',
  '$guidance','$category','$license_type','$license_profile',	'$license_style'	,'$owner_name',
  '$SPDX_key','$publication_year','$keywords','$special_obligations','$homepage_url',
  '$text_url','$osi_url','$faq_url','$guidance_url','$other_urls','$owner_name',
  '$owner_homepage_url','$owner_type','$contact_information','$alias','$owner_notes')";

  if($nexb_key == ""){
   echo "* No license key";
  }
  $result_insert_nexb=mysqli_query($con,$insert_nexb);
//check query
if(! $result_insert_nexb ) {
    echo
    "<form action='admin.php' method='post'>".
    "<input type='hidden' name='username' value ='admin'>".
    "<input type='hidden' name='password' value='password'>".
    "<input class='btn btn-danger' type='submit' value ='Back'></form>";
		die("<div class='alert alert-danger'><strong>Error : </strong> Could not insert data into nexb_license:</div>" . mysql_error());
 }
 else{
   echo
   "<form action='admin.php' method='post'>".
   ...
   $nexb_key." already inserted into nexb_license table."."</div>";
 }
mysqli_close($con);
 ?>
 </body>

...
//query delete in match_license table
$delete_match = "DELETE FROM match_license WHERE olex_key ='$key' ";

$result_delete_match = mysqli_query($con,$delete_match);
//check query
 if(! $result_delete_match) {
     ...
     "<input class='btn btn-danger' type='submit' value ='Back'></form>";
 		die("<div class='alert alert-danger'><strong>Error : </strong> Could not delete data into match_license:</div>" . mysql_error());
  }
  else{
    echo
    "<form action='admin.php' method='post'>".
    ...
    $key." already deleted into match_license table."."</div>";
  }
 mysqli_close($con);
  ?>
</body>
